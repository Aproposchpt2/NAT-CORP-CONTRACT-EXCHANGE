...
// Owner policy: one OpenAI discovery request per publisher connector per run.
// Weekly/owner-triggered acquisition must never fan out into automatic passes.
const MAX_PASSES = 1;
...
async function discoverCandidates({ apiKey, model, scope, excludedUrls }) {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      ...
      input: [
        { role: 'system', content: `You are a meticulous government procurement acquisition agent. Stay inside the assigned verified publisher, use live authoritative public sources, never fabricate a contract record, and make exactly one discovery pass for the publisher per owner-triggered run.` },
