...
function hostMatches(hostname,suffixes){const host=String(hostname||'').toLowerCase();return suffixes.some(s=>host===String(s).replace(/^\./,'').toLowerCase()||host.endsWith(String(s).toLowerCase()))}
function hostAllowed(hostname){return hostMatches(hostname,ALLOWED_HOST_SUFFIXES)}
function hostAllowedForScope(hostname,scope){const suffixes=Array.isArray(scope?.allowed_host_suffixes)?scope.allowed_host_suffixes:[];return hostAllowed(hostname)&&(!suffixes.length||hostMatches(hostname,suffixes))}
function normalizedName(value){return String(value||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()}
function agencyAllowedForScope(value,scope){const agency=normalizedName(value);if(!agency||agency.includes('unknown issuing organization'))return false;const publishers=Array.isArray(scope?.publishers)?scope.publishers:[];if(!publishers.length)return true;if(String(scope?.id||'').toUpperCase()==='CA_JAGGAER'&&/(california state university|state university|cal state)/.test(agency))return true;return publishers.some(p=>[p.name,...(p.aliases||[])].some(label=>{const expected=normalizedName(label);return expected.length>=5&&(agency===expected||agency.includes(expected)||expected.includes(agency))}))}
...
async function verifyPublicUrl(value,{scope=null}={}){
  const url=canonicalUrl(value),parsed=new URL(url);
  if(!hostAllowedForScope(parsed.hostname,scope))throw new Error('SOURCE_HOST_OUTSIDE_ASSIGNED_PUBLISHER_SCOPE');
  ...
  if(!response.ok){const allowed=browserDependentAccessAllowed(scope?.id,parsed.hostname,response.status);await response.body?.cancel().catch(()=>{});if(allowed)return url;throw new Error(`AUTHORITATIVE_URL_HTTP_${response.status}`)}
  const finalUrl=canonicalUrl(response.url||url);if(!hostAllowedForScope(new URL(finalUrl).hostname,scope))throw new Error('SOURCE_REDIRECTED_OUTSIDE_ASSIGNED_PUBLISHER_SCOPE');await response.body?.cancel().catch(()=>{});return finalUrl;
}
...
export async function runOneCallOpenAIDiscovery({scope,target=DISCOVERY_TARGET,onProgress=async()=>{},stateName,runId,taskSessionId}={}){
  ...
  for(const candidate of batch.candidates){
    try{
      ...
      const url=await verifyPublicUrl(rawUrl,{scope});
      const closesAt=validClosingDate(candidate.closes_at);
      if(!clean(candidate.title,500)||!clean(candidate.agency_name,300))throw new Error('TITLE_AND_AGENCY_REQUIRED');if(!agencyAllowedForScope(candidate.agency_name,scope))throw new Error('ISSUING_AGENCY_OUTSIDE_ASSIGNED_PUBLISHER_SCOPE');
