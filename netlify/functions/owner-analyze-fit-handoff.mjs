...
async function loadContext(outreachId) {
  ...
  if (safe(outreach.response_class).toUpperCase() !== 'INTERESTED') throw new Error('Owner Analyze Fit handoff requires an INTERESTED outreach response.');
  if (!outreach.candidate_id || !outreach.opportunity_id) throw new Error('Interested outreach is missing contractor or contract linkage.');
  ...
  if (!candidate) throw new Error('Interested contractor candidate could not be loaded.');
  if (!opportunity) throw new Error('Selected contract could not be loaded.');
  return { outreach, candidate, opportunity };
}

async function ensureServiceRequest(context) {
  const prior = context.outreach?.provider_payload?.owner_analyze_fit_handoff;
  if (prior?.status === 'sent' && prior?.client_url && prior?.request_id) return prior;

  const accessToken = randomBytes(32).toString('base64url');
  const expiresAt = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString();
  const metadata = {
    source: 'otf_interested_owner_handoff',
  ...
  if (!request) throw new Error('Analyze Fit service request could not be created.');

  const clientUrl = `${PUBLIC_BASE()}/analyze-fit-request?request=${encodeURIComponent(request.request_id)}&token=${encodeURIComponent(accessToken)}`;
  return {
    status: 'created',
...
async function sendOwnerEmail(context, handoff) {
  if (handoff.status === 'sent') return handoff;
  const key = env('RESEND_API_KEY');
  if (!key) throw new Error('RESEND_API_KEY is unavailable for the owner Analyze Fit handoff.');
  const to = ownerEmail();
  if (!to) throw new Error('NAT-CORP owner notification email is not configured.');

  const business = context.candidate.business_name || context.outreach.business_name || 'Interested contractor';
  ...
  const text = `NAT-CORP Interested Contractor — Analyze Fit Link Ready\n\nContractor: ${business}\nContact: ${context.candidate.contact_name || context.outreach.contact_name || 'Unavailable'}\nEmail: ${context.candidate.contact_email || context.outreach.contact_email || 'Unavailable'}\nPhone: ${context.candidate.contact_phone || 'Unavailable'}\n\nContract: ${title}\nReference: ${reference}\nIssuing organization: ${issuer}\nResponse deadline: ${deadline}\n\nClient-safe Analyze Fit URL:\n${handoff.client_url}\n\nForward this URL to the interested contractor. It opens the contract-specific Opportunity-to-Fulfillment Analyze Fit confirmation page with contractor and contract context precompleted.`;

  const html = `<!doctype html><html><body style="margin:0;background:#f3f5f8;font-family:Arial,Helvetica,sans-serif;color:#14213d"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f5f8;padding:24px 12px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#fff;border:1px solid #d9e0ea;border-radius:14px;overflow:hidden"><tr><td align="center" style="padding:26px 24px 12px"><img src="cid:apropos-group-logo" width="210" alt="APROPOS GROUP LLC" style="display:block;max-width:210px;width:100%;height:auto;border:0"></td></tr><tr><td style="padding:0 30px 30px;font-size:15px;line-height:1.55"><p style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#56709a;font-weight:700">NAT-CORP Owner Handoff</p><h1 style="font-family:Georgia,serif;color:#0d2a57;font-size:28px;line-height:1.15">Interested contractor Analyze Fit link is ready.</h1><p><strong>Contractor:</strong> ${htmlEsc(business)}<br><strong>Contact:</strong> ${htmlEsc(context.candidate.contact_name || context.outreach.contact_name || 'Unavailable')}<br><strong>Email:</strong> ${htmlEsc(context.candidate.contact_email || context.outreach.contact_email || 'Unavailable')}<br><strong>Phone:</strong> ${htmlEsc(context.candidate.contact_phone || 'Unavailable')}</p><p><strong>Contract:</strong> ${htmlEsc(title)}<br><strong>Reference:</strong> ${htmlEsc(reference)}<br><strong>Issuing organization:</strong> ${htmlEsc(issuer)}<br><strong>Response deadline:</strong> ${htmlEsc(deadline)}</p><p><a href="${handoff.client_url}" style="display:inline-block;background:#0d2a57;color:#fff;text-decoration:none;padding:13px 18px;border-radius:8px;font-weight:700">Open Client Analyze Fit Link</a></p><p>Forward this client-safe URL to the interested contractor. The link opens the contract-specific Opportunity-to-Fulfillment Analyze Fit confirmation page with the contractor and selected contract precompleted.</p><p style="font-size:12px;color:#66758a;word-break:break-all">${htmlEsc(handoff.client_url)}</p></td></tr></table></td></tr></table></body></html>`;
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: resendFrom(),
      to: [to],
      subject: `Interested contractor Analyze Fit link: ${business}`,
      text,
      html,
      attachments: [aproposLogoAttachment()],
      reply_to: to,
      tags: [
        { name: 'service', value: 'natcorp-owner-handoff' },
  ...
  if (!response.ok) throw new Error(`Owner Analyze Fit email failed (${response.status}): ${data.message || 'unknown error'}`);
  ...
  await db('natcorp_outreach_events', 'PATCH', `?outreach_id=eq.${encodeURIComponent(context.outreach.outreach_id)}`, {
    provider_payload: {
      ...(context.outreach.provider_payload || {}),
      owner_analyze_fit_handoff: completed,
    },
...
export default async function handler(req) {
  ...
  try {
    ...
    const handoff = await ensureServiceRequest(context);
    const completed = await sendOwnerEmail(context, handoff);
    return json(200, {
      ok: true,
      handoff: completed,
      contractor: {
      ...
      contract: {
        opportunity_id: context.opportunity.id,
        reference: context.opportunity.pdas_record_id || context.opportunity.id,
        title: context.opportunity.title,
      },
    });
  } catch (error) {
    console.error('[owner-analyze-fit-handoff]', error);
    return json(500, { ok: false, error: safe(error?.message, 700) || 'Owner Analyze Fit handoff failed.' });
  }
}

export const config = { path: '/api/owner-analyze-fit-handoff' };
