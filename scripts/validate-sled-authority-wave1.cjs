...
if(!hub.includes('"dateModified":"2026-08-21"'))failures.push('hub missing freshness signal');
if(failures.length){console.error('[natcorp-sled-wave1] Validation failed:');failures.forEach(f=>console.error(`- ${f}`));process.exit(1);}console.log('[natcorp-sled-wave1] PASS — guide hub + 4 SLED authority pages are canonical, indexable, sitemap-listed, current-property routed, retired-property clean, and GEO-semantic ready.');